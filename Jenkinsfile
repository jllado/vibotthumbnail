pipeline {
    ...
    stages {
        stage ('Build') {
            steps {
                sh 'export APP_PATH=/opt/servers/jenkins/workspace/vibot-thumbnail && docker-compose -f docker-compose.build.yml up --build --exit-code-from vibot-thumbnail-build'
            }
        }
        stage ('Deploy') {
            steps {
